...
class ReviewEngine:
    ...
    def classify_move(
        ...
        if (mate_before is not None and mate_before > 0 and mate_after is None) or (score_before >= 300 and cp_loss >= 180):
            return 'MISS', 'OPORTUNIDAD PERDIDA'

        # 3. BRILLIANT: Sacrifice or exceptional tactic with strong evaluation retained
        ...
        if is_sacrificed and cp_loss <= 20 and score_after >= -50:
            return 'BRILLIANT', 'BRILLANTE'

        # 4. BEST: Matches Stockfish top choice or cp_loss < 10
        if (best_uci and move.uci() == best_uci) or cp_loss < 10:
            return 'BEST', 'MEJOR'

        # 5. EXCELLENT: 10 <= cp_loss <= 30
        if cp_loss <= 30:
            return 'EXCELLENT', 'EXCELENTE'

        # 6. GOOD: 30 < cp_loss <= 75
        if cp_loss <= 75:
            return 'GOOD', 'BUENA'

        # 7. INACCURACY: 75 < cp_loss <= 150
        if cp_loss <= 150:
            return 'INACCURACY', 'INEXACTITUD'

        # 8. MISTAKE: 150 < cp_loss <= 300
        if cp_loss <= 300:
            return 'MISTAKE', 'ERROR'

        # 9. BLUNDER: cp_loss > 300 or win-to-loss swing
